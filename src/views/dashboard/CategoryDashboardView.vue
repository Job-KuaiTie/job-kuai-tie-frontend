...
<template>
    <Dialog v-model:visible="CategoryDialog" modal header="新增分類" :style="{ width: '25rem' }">
      ...
      <div class="flex items-center gap-4 mb-4">
        ...
        <Select v-model="categoryData.color" :options="colorOptions" optionLabel="label" optionValue="value" class="w-full md:w-56">
      ...
      <div class="flex items-center gap-4 mb-4">
          ...
          <Textarea id="description" class="flex-auto" v-model="categoryData.description" autocomplete="off" rows="5" cols="30"/>
      </div>
  ...
  <Dialog v-model:visible="deleteCategoryDialog" :style="{ width: '25rem' }" header="確認刪除？" :modal="true">
          <div class="flex items-center gap-4">
  ...
  <div class="card">
    ...
    <DataTable :value="categories" tableStyle="min-width: 30rem"  :loading="loading" dataKey="id">
      ...
      <Column field="name" header="分類名稱" sortable>
      </Column>
      ...
      <Column field="description" header="分類描述" sortable>
      </Column>
      <Column :exportable="false">
        <template #body="slotProps">
            <Button icon="pi pi-pencil" outlined rounded class="mr-2" @click="editCategory(slotProps.data)" />
            <Button icon="pi pi-trash" text severity="danger" @click="confirmDeleteCategory(slotProps.data)" />
        </template>
    </Column>
